...
names(dat)
dim(dat)[1] #vector that shows rows/columns vector of 2 digits
nrow(dat)
head(dat)
...
dat$Petal.Width <- as.character(dat$Petal.Width)
class(dat$Species)
...
dat$Sepal.Width <- as.numeric(dat$Sepal.Width)
...
big_area_iris <- subset.data.frame(dat,dat$Sepal.Area > 20,drop=FALSE)
